        ...
        stage('sonar') {
                ...
                withSonarQubeEnv('sonarqube-server') {
                    if (ejecucion.checkOs() == 'Windows') {
                        ...
                        bat "${scannerHomeW}/bin/sonar-scanner -Dsonar.projectName=${env.SONAR_NOMBRE_EJECUCION} -Dsonar.projectKey=pipeline-devops-labm3-gradle -Dsonar.sources=src -Dsonar.java.binaries=build/libs"
                    } else {
                        sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName=${env.SONAR_NOMBRE_EJECUCION} -Dsonar.projectKey=pipeline-devops-labm3-gradle -Dsonar.sources=src -Dsonar.java.binaries=build/libs"
                    }
                }
                }
        }
