    ...
    stage('sonar') {
      if (env.PSTAGE == env.STAGE_NAME || env.PSTAGE == 'ALL') {
        ...
        withSonarQubeEnv('sonarqube-server') {
            ...
            bat "${scannerHomeW}/bin/sonar-scanner -Dsonar.projectName=${env.SONAR_NOMBRE_EJECUCION} -Dsonar.projectKey=pipeline-devops-labm3-maven -Dsonar.sources=src -Dsonar.java.binaries=build"
            } else {
            sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectName=${env.SONAR_NOMBRE_EJECUCION} -Dsonar.projectKey=pipeline-devops-labm3-maven -Dsonar.sources=src -Dsonar.java.binaries=build"
            }
        }
      }
    }
